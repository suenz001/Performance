...
const ResultList: React.FC<ResultListProps> = ({ records, duplicateNames = [] }) => {
    ...
    <div className="mt-8">
      <div className="flex justify-between items-end mb-4">
        ...
        {duplicateNames.length > 0 && (
          <p className="text-[11px] text-amber-600 font-medium bg-amber-50 px-2 py-1 rounded border border-amber-200 animate-pulse">
            ⚠️ 橘色標記表示該姓名出現多次，請核對是否為同一人或同名。
          </p>
        )}
      </div>
      
      <div className="overflow-hidden shadow-sm ring-1 ring-black ring-opacity-5 rounded-lg border border-slate-200">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-300">
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {records.map((record) => {
                const isDuplicate = duplicateNames.includes(record.name.trim());
                return (
                  <tr 
                    key={record.id} 
                    className={`transition-colors ${isDuplicate ? 'bg-amber-50/70 hover:bg-amber-100/80' : 'hover:bg-slate-50'}`}
                  >
                    ...
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-700">
                      <div className="flex items-center space-x-2">
                        <span className={isDuplicate ? 'font-bold text-amber-900' : ''}>{record.name}</span>
                        {isDuplicate && (
                    ...
                    <td className="whitespace-nowrap px-3 py-4 text-sm text-slate-700 font-mono">
                      <span className={isDuplicate ? 'bg-amber-200/50 px-1.5 py-0.5 rounded' : ''}>
                        {record.supervisorRating}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-3 py-4 text-right text-xs text-slate-400">
                      <span title={record.fileName} className="cursor-help underline decoration-dotted">
                        {record.fileName.length > 15 ? record.fileName.substring(0, 12) + '...' : record.fileName}
                      </span>
                      <span className="ml-1">(P.{record.pageNumber})</span>
                    </td>
                  </tr>
                );
              })}
